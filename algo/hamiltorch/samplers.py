...
def hamiltonian(params, momentum, log_prob_func, jitter=0.01, normalizing_const=1., softabs_const=1e6, explicit_binding_const=100, inv_mass=None, ham_func=None, sampler=Sampler.HMC, integrator=Integrator.EXPLICIT, metric=Metric.HESSIAN):
    ...
    if sampler == Sampler.HMC:
        ...
        return hamiltonian, potential, kinetic 
...
def sample(log_prob_func, params_init, num_samples=10, num_steps_per_sample=10, step_size=0.1, burn=0, jitter=None, inv_mass=None, normalizing_const=1., softabs_const=None, explicit_binding_const=100, fixed_point_threshold=1e-5, fixed_point_max_iterations=1000, jitter_max_tries=10, sampler=Sampler.HMC, integrator=Integrator.IMPLICIT, metric=Metric.HESSIAN, debug=False, desired_accept_rate=0.8, store_on_GPU = True):
    ...
    result = {'hamitonian':[],'potential':[],'kinetic':[],'new_hamitonian':[],'new_potential':[],'new_kinetic':[],'accept_rate':0}
    for n in range(num_samples):
        ...
        try:
            ...
            ham,potential,kinetic = hamiltonian(params, momentum, log_prob_func, jitter=jitter, softabs_const=softabs_const, explicit_binding_const=explicit_binding_const, normalizing_const=normalizing_const, sampler=sampler, integrator=integrator, metric=metric, inv_mass=inv_mass)
            result['hamitonian'].append(ham)
            result['potential'].append(potential)
            result['kinetic'].append(kinetic)
            print('old ham', ham)
            # the place decrease the speed
            ...
            if sampler == Sampler.RMHMC and integrator == Integrator.EXPLICIT:
                ...
                new_ham,new_potential,new_kinetic = hamiltonian(params, momentum, log_prob_func, jitter=jitter, softabs_const=softabs_const, explicit_binding_const=explicit_binding_const, normalizing_const=normalizing_const, sampler=sampler, integrator=integrator, metric=metric, inv_mass=inv_mass)
                print('new hamitonian', new_ham)
    ...
    result['accept_rate'] = 1 - num_rejected/num_samples
    if NUTS and debug == 2:
        return list(map(lambda t: t.detach(), ret_params)), step_size
    elif debug == 2:
        return list(map(lambda t: t.detach(), ret_params)), 1 - num_rejected/num_samples, result
    else:
        return list(map(lambda t: t.detach(), ret_params)), result
...
def sample_model(model, x, y, params_init, model_loss='multi_class_linear_output' ,num_samples=10, num_steps_per_sample=10, step_size=0.1, burn=0, inv_mass=None, jitter=None, normalizing_const=1., softabs_const=None, explicit_binding_const=100, fixed_point_threshold=1e-5, fixed_point_max_iterations=1000, jitter_max_tries=10, sampler=Sampler.HMC, integrator=Integrator.IMPLICIT, metric=Metric.HESSIAN, debug=False, tau_out=1.,tau_list=None, store_on_GPU = True, desired_accept_rate=0.8):
    ...
    log_prob_func = define_model_log_prob(model, model_loss, x, y, params_flattened_list, params_shape_list, tau_list, tau_out, normalizing_const=normalizing_const,  prior_scale=2, device = device)
